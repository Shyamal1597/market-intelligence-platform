...
export function WatchlistManager({ watchlist, onRemove, onAdd, onReset, adding }: Props) {
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap border border-border rounded-lg px-3 py-2 bg-surface">
      <span className="text-[10px] font-mono text-muted tracking-widest mr-1">COVERAGE</span>
      {watchlist.map((e) => (
        <button
          key={e.symbol}
          onClick={() => onRemove(e.symbol)}
          className="flex items-center gap-1 px-2 py-0.5 rounded text-xs font-mono bg-border text-primary hover:bg-danger/20 hover:text-danger transition-colors group"
        >
          {e.symbol}
      ...
      <div className="flex items-center gap-1 ml-auto">
        <input
          ...
          className="w-28 px-2 py-0.5 text-xs font-mono bg-background border border-border rounded text-primary placeholder-muted focus:outline-none focus:border-amber/60 transition-colors"
        />
        <button
          onClick={handleAdd}
